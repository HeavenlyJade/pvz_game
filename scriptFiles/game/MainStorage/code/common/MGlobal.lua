...
local  gg = {
    ...
    ---@type table
    server_player_bag_data = {          --玩家背包数据管理
        -- [ 10001 ] =  {}
        -- [ 10002 ] =  {}
    },
    ...
    client_bag_index = {},          --服务器同步给玩家的背包放置数据   ---@type table
    client_bag_items = {},          --服务器同步给玩家的背包物品详情   ---@type table
    client_player_data = {          --玩家数据 (需要从服务器同步)      ---@type table        
...
--按照uin获得玩家昵称
function gg.getNickNameByUin(uin_)
    if  gg.server_players_list[ uin_ ] then
        return gg.server_players_list[ uin_ ].info.nickname;
    end
    return '神秘人' .. uin_
end
...
--发送系统消息
function gg.send_msg( pps_ )
    -- { type='defeat', uin=self.info.uin, from_uin=from_uin_  }
    if  pps_.type == 'defeat' then
        --gg.getNickNameByUin( pps_.from_uin )
        --gg.getNickNameByUin( pps_.uin )
        --local chat = game:GetService("Chat")
        --chat:SendSystemMsg ( ''.. gg.getNickNameByUin( pps_.from_uin )  .. ' 击落 ' .. gg.getNickNameByUin( pps_.uin ), 1 )
    end
end
...
function gg.create_uuid( pre_ )
    ...
    return  pre_ ..  gg.uuid_start  .. '_' .. ( os.clock()*1000 + math.random(1, 1000) ) % 1000 .. '_' .. math.random( 10000, 99999 )
end
...
--浅拷贝 不拷贝meta
function gg.table_value_copy(ori_tab)
    ...
    for i, v in pairs(ori_tab) do
        if  type(v) == "table" then
            new_tab[i] = gg.table_value_copy(v)
        else
            new_tab[i] = v
        end
    end
...
function gg.fast_in_distance( dir_, len )
    if  dir_.x > 0-len and dir_.x < len and
        dir_.y > 0-len and dir_.y < len and
        dir_.z > 0-len and dir_.z < len then
        return true
    else
        return false
    end
...
function gg.playerTeleportToPostion( player_, pos_, scene_name_ )
    ...
    if  nessesary_base_obj_ then
        local listener
        listener = nessesary_base_obj_.LoadFinish:connect(
            function(ret)
