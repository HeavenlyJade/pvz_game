...
local function UpdateShake(dt)
    ...
    if _shakeData.elapsedtime < _shakeData.duration then
        ...
        _shakeData.rotDelta = gg.vec.Multiply(Vector3.New(rx * _shakeData.rotX, ry * _shakeData.rotX, rz * _shakeData.rotX), _shakeData.strength * easingValue)
        ...
        _shakeData.posDelta = gg.vec.Multiply(Vector3.New(px * _shakeData.posX, py * _shakeData.posY, pz * _shakeData.posZ), _shakeData.strength * easingValue)
    else
...
local function StartUpdateLoop()
    if not _updateConnection then
        _updateConnection = game:GetService("RunService").RenderStepped:Connect(Update)
    end
end
