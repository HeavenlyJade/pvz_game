...
---@class BaseTask
local BaseTask = CommonModule.Class('BaseTask')
...
-- 初始化任务
function BaseTask:OnInit(taskData)
    ...
    -- 任务状态
    self.status = "未接取"         -- 可能的状态: 未接取, 进行中, 待领取, 已完成, 已失败
...
-- 接取任务
function BaseTask:Accept(player)
...
-- 更新任务目标进度
function BaseTask:UpdateObjective(player, objectiveIndex, progress)
...
-- 检查任务是否可以完成
function BaseTask:CheckCompletion(player)
    if self.status ~= "进行中" then
...
-- 完成任务并领取奖励
function BaseTask:Complete(player)
    ...
    if self.config.next_quest then
        CommandManager:ExecuteCommand("任务 设置 " .. self.config.next_quest .. " 状态 = 进行中", player)
    end
...
-- 失败任务
function BaseTask:Fail(player, reason)
    if self.status ~= "进行中" then
        return false, "无法标记失败：任务不在进行中"
    end
...
-- 设置对话进度
function BaseTask:SetDialogueProgress(player, progress)
...
-- 获取对话内容
function BaseTask:GetDialogue(progress)
...
-- 获取目标描述
function BaseTask:GetObjectiveDescription(objective)
