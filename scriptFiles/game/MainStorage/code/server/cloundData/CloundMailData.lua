...
---@class CloundMailData
local CloundMailData = {
...
function CloundMailData:GenerateMailId(prefix)
...
---@return table 玩家邮件数据
function CloundMailData:LoadPlayerMail(uin)
...
---@return boolean 是否成功
function CloundMailData:SavePlayerMail(uin, force)
...
---@param uin number 玩家ID
---@return table 玩家邮件数据
function CloundMailData:GetPlayerMail(uin)
...
---@return string 邮件ID
function CloundMailData:AddPlayerMail(uin, mailData)
...
--- 加载全服邮件
---@return table 全服邮件数据
function CloundMailData:LoadGlobalMail()
...
---@param force boolean 是否强制保存
---@return boolean 是否成功
function CloundMailData:SaveGlobalMail(force)
...
--- 获取全服邮件数据
---@return table 全服邮件数据
function CloundMailData:GetGlobalMail()
    if not self.global_mail_cache then
...
---@param mailData table 邮件数据
---@return string 邮件ID
function CloundMailData:AddGlobalMail(mailData)
    if not self.global_mail_cache then
        self:LoadGlobalMail()
    end
...
---@param uin number 玩家ID
---@return table 位图数据
function CloundMailData:LoadPlayerMailBitmap(uin)
    -- 从云存储加载玩家邮件位图
...
---@param force boolean 是否强制保存
---@return boolean 是否成功
function CloundMailData:SavePlayerMailBitmap(uin, force)
    -- 保存玩家邮件位图到云存储
...
---@param uin number 玩家ID
---@return table 位图数据
function CloundMailData:GetBitmap(uin)
    -- 获取玩家位图，如不存在则加载
...
---@return boolean 是否成功
function CloundMailData:SetMailBit(uin, mailId, bitValue)
    -- 设置特定邮件的位状态
...
---@param mailId string 邮件ID
---@return number 位值
function CloundMailData:GetMailBit(uin, mailId)
    -- 获取特定邮件的位状态
    local bitmap = self:GetBitmap(uin)
...
--- 玩家登录事件处理
---@param uin number 玩家ID
function CloundMailData:OnPlayerLogin(uin)
    -- 加载个人邮件
...
--- 玩家登出事件处理
---@param uin number 玩家ID
function CloundMailData:OnPlayerLogout(uin)
    -- 保存个人邮件
    self:SavePlayerMail(uin, true)
